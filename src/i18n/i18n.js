...
(function () {
  ...
  var KEY = 'u12-locale';
  // Read lazily: in the bundled single-file build the helmet scripts become blob
  // URLs and may execute out of authored order, so strings.js can land after this
  // file. Snapshotting the packs here would freeze an empty catalogue forever.
  function getPacks() { return window.U12_I18N || {}; }
  function hasPacks() { var p = getPacks(); return !!(p && p[BASE] && Object.keys(p[BASE]).length); }
  ...
  var api = {
    ...
    t: function (key, vars) {
      ...
      if (typeof v !== 'string') {
        if (this.ready && window.console) console.warn('[i18n] missing key: ' + key);
        return '';
      }
      return fill(v, vars);
    },
  ...
  // Late-arriving packs: rebuild and tell the app to re-render.
  function announce() {
    api.ready = true;
    api.build();
  ...
  function waitForPacks(tries) {
    if (hasPacks()) return announce();
    if (tries <= 0) return;
    setTimeout(function () { waitForPacks(tries - 1); }, 25);
  }
  ...
  window.I18N = api;
  if (!api.ready) {
    waitForPacks(400); // up to ~10s, then give up quietly
    document.addEventListener('DOMContentLoaded', function () { if (!api.ready && hasPacks()) announce(); });
    window.addEventListener('load', function () { if (!api.ready && hasPacks()) announce(); });
  }
})();
